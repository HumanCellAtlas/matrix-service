...
def worker_handler(event: dict, context: dict):
    ...
    Input:
        event: (dict) keys expected 'request_hash': str, 'worker_chunk_spec': dict
    ...
    assert 'request_id' in event and 'request_hash' in event and 'worker_chunk_spec' in event
    ...
    worker = Worker(event['request_id'], event['request_hash'])
