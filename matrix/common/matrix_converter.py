...
def zarr_to_loom(zarr_root):
    ...
    row_attrs = {}
    row_attrs["cell_id"] = zarr_root.cell_id[:]
    for cell_metadata_idx in range(zarr_root.cell_metadata_name.shape[0]):
        metadata_name = zarr_root.cell_metadata_name[cell_metadata_idx]
        row_attrs[metadata_name] = \
            zarr_root.cell_metadata[:, cell_metadata_idx]

    # Set the gene metadata as loom col attrs
    col_attrs = {}
    col_attrs["gene_id"] = zarr_root.gene_id[:]
    try:
    ...
    loompy.create(loom_path, zarr_root.expression[:], row_attrs, col_attrs)
...
def main(args):
    ...
    upload_converted_matrix(local_converted_path, args.dest_path)
